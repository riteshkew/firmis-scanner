...
description: All 16 threat categories detected by Firmis — rule counts, severity ranges, OWASP LLM Top 10 mappings, MITRE ATT&CK mappings, and per-category detection details.
---

import { Aside } from '@astrojs/starlight/components';

> The authoritative reference for Firmis's 16 threat categories. Use this page to understand what each category detects, how findings are identified, and how they map to OWASP LLM Top 10 and MITRE ATT&CK for ML.
...
Secret detection is the largest category by rule count. It covers hardcoded credentials across 30+ cloud providers, SaaS APIs, infrastructure services, and generic token formats. This category is exempt from the 0.15x document multiplier — secrets in `.env.example` and `README.md` files are still reported.
...
<Aside type="note">
  Secret detection rules are exempt from the document multiplier. A hardcoded secret in `.env.example` or a README code sample is still flagged because it may be committed to source control and discovered by attackers scanning public repositories.
</Aside>
...
- [Threat Model](/concepts/threat-model) — narrative descriptions of each category
...
- [Security Model](/reference/security-model) — what Firmis detects and what it does not
