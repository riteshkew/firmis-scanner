...
description: SARIF 2.1.0 output format produced by Firmis — field mappings, generation commands, GitHub Security tab integration, and a complete example.
---

import { Aside } from '@astrojs/starlight/components';

> Firmis outputs standard SARIF 2.1.0, the Static Analysis Results Interchange Format. SARIF is the format required by the GitHub Security tab, accepted by VS Code's SARIF Viewer, and supported by most CI security dashboards.
...
- [GitHub Actions integration](/integrations/github-actions) — full workflow example
- [CycloneDX BOM](/reference/cyclonedx-bom) — agent BOM output format
- [Threat Categories](/reference/threat-categories) — what each ruleId maps to
